...
export const ItemsSlider = ({ sliderData }) => {
  ...
  const slideList = sliderData.map((slide) => {
    return (
      <SwiperSlide key={slide.id}>
        <ItemOfSlider
          image={
            slide.attributes.imagesSlider.data
              ? `${url}${slide.attributes.imagesSlider.data[0].attributes.url}`
              : noImage
          }
          title={slide.attributes.title}
          oldPrice={slide.attributes.oldPrice}
          relevantPrice={slide.attributes.relevantPrice}
          key={slide.id}
          categoryNav={slide.attributes.categoryName}
          id={slide.id}
        />
      </SwiperSlide>
    );
  ...
  return (
    <div style={{ width: "100%" }}>
      {/*<h3 className={slider.title}>Ми рекомендуємо</h3>*/}
      <div
